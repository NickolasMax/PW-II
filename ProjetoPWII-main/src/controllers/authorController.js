...
import author from "../entities/autor.js";
...
route.post("/", async (request, response) => {
    const { nome_autor, nasc_autor, nacionalidade } = request.body;

    if(nome_autor.length < 1){
        return response.status(400).send({ "response": "O nome do autor deve conter no mínimo 1 caractere." });
    }

   if(nasc_autor.length !== 10){
        return response.status(400).send({"response":"o campo de data deve ser no fromato dd/mm/aaaa"});
    }

    if(nacionalidade.length < 1){
        return response.status(400).send({ "response": "Digite no mínimo 1 caractere na nacionalidade" });
    }

    const newAuthor = authorRepository.create({ nome_autor, nasc_autor, nacionalidade });
...
route.put('/:codigo_autor',async(request, response)=> {
    ...
    if(nasc_autor.length !== 10){
        return response.status(400).send({"response":"O formato deve ser dd/mm/aaaa."})
    }
...
route.delete('/:codigo_autor',async (request, response) => {
    const {codigo_autor} = request.params;
